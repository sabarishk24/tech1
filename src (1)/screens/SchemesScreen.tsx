      ...
      <Modal
        ...
        footer={
          selectedScheme?.isEligible ? (
            <button className="w-full py-3.5 bg-primary text-white font-bold rounded-[14px] hover:bg-primary-dark transition-colors">
              {t('schemes.apply')} →
            </button>
          ) : undefined
        }
    ...
    <Card onClick={onSelect} className={scheme.isEligible ? 'border-2 border-primary/30' : ''}>
      ...
      {scheme.isEligible && (
        <div className="mt-3 border-t border-border/50 pt-3">
          <button onClick={e => { e.stopPropagation(); }} className="w-full py-2 bg-primary text-white text-sm font-bold rounded-[12px] hover:bg-primary-dark transition-colors">
            {t('schemes.apply')}
          </button>
        </div>
      )}
