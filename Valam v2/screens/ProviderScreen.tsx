...
export default function ProviderScreen() {
  ...
  const pending = requests.filter(r => r.status === 'pending').length + enquiries.length;
          ...
          <>
            {enquiries.length > 0 && (
              <div className="flex flex-col gap-3 mb-1">
                <p className="text-xs font-bold text-amber-700 uppercase tracking-wide">Market Enquiries</p>
                {enquiries.map(enq => (
                  <Card key={enq.id} className="border-2 border-amber-200 bg-amber-50/40">
                    <div className="flex items-start gap-3">
                      <div className="w-10 h-10 rounded-[12px] bg-amber-100 flex items-center justify-center text-amber-700 font-black text-sm flex-shrink-0">
                        {enq.farmer[0]}
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-bold text-text">{enq.farmer}</span>
                          <span className="px-2 py-0.5 rounded-full bg-amber-200 text-amber-800 text-[10px] font-bold">Enquiry</span>
                        </div>
                        <p className="text-xs text-muted mt-0.5">Interested in selling <span className="font-semibold text-text">{enq.quantity} qtl of {enq.crop}</span></p>
                        <p className="text-xs text-muted">At: {enq.mandi} · {enq.date}</p>
                      </div>
                    </div>
                    <div className="flex gap-2 mt-3">
                      <button className="flex-1 py-2 rounded-[10px] border border-border text-muted text-xs font-bold hover:bg-surface-2 transition-colors">
                        Ignore
                      </button>
                      <button className="flex-1 py-2 rounded-[10px] bg-primary text-white text-xs font-bold hover:bg-primary-dark transition-colors">
                        Contact Farmer
                      </button>
                    </div>
                  </Card>
                ...
                <div className="border-t border-border/50 pt-1">
                  <p className="text-xs font-bold text-muted uppercase tracking-wide">Booking Requests</p>
                </div>
              </div>
            )}
            {requests.map(req => (
